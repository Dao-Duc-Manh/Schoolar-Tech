...
export default function AIReportSettings() {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {criteriaDefinitions.map((criteria) => (
              <div key={criteria.id} className="bg-gray-50 p-6 rounded-xl">
                <h3 className="font-bold text-gray-900 mb-2">{criteria.name}</h3>
                <p className="text-sm text-gray-600">{criteria.description}</p>
                <div className="mt-3 pt-3 border-t border-gray-200">
                  <p className="text-xs font-medium text-gray-500 uppercase mb-2">Các yếu tố được đo lường:</p>
                  <div className="flex flex-wrap gap-2">
                    {criteria.id === 'engagement' && (
                      <>
                        <span className="px-2 py-1 bg-white rounded-lg text-xs border">Số lần phát biểu</span>
                        <span className="px-2 py-1 bg-white rounded-lg text-xs border">Tham gia thảo luận</span>
                        <span className="px-2 py-1 bg-white rounded-lg text-xs border">Nộp bài đúng hạn</span>
                      </>
                    )}
                    {criteria.id === 'academic' && (
                      <>
                        <span className="px-2 py-1 bg-white rounded-lg text-xs border">Điểm bài tập</span>
                        <span className="px-2 py-1 bg-white rounded-lg text-xs border">Điểm thi</span>
                        <span className="px-2 py-1 bg-white rounded-lg text-xs border">Tiến độ học</span>
                      </>
                    )}
                    {criteria.id === 'sentiment' && (
                      <>
                        <span className="px-2 py-1 bg-white rounded-lg text-xs border">Phân tích văn bản</span>
                        <span className="px-2 py-1 bg-white rounded-lg text-xs border">Mức độ tập trung</span>
                        <span className="px-2 py-1 bg-white rounded-lg text-xs border">Tương tác tích cực</span>
                      </>
                    )}
                  </div>
                </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="text-2xl font-bold text-gray-900">Cài đặt nâng cao</h2>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium text-gray-900">Phân tích thời gian thực</p>
                <p className="text-sm text-gray-500">Cập nhật insights mỗi khi có dữ liệu mới</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium text-gray-900">Gợi ý tự động</p>
                <p className="text-sm text-gray-500">AI đề xuất can thiệp khi phát hiện vấn đề</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" defaultChecked className="sr-only peer" />
                <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-100 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium text-gray-900">Báo cáo tự động</p>
                <p className="text-sm text-gray-500">Gửi báo cáo định kỳ qua email</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
